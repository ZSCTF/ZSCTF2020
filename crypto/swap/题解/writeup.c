...
void encrypt(char *str, int str_len, char *key, int key_len, char *result)
{
    ...
    for (int i = 0; i < str_len; i++)
    {
        ...
        if (i + 1 <= str_len - 1)
        {
            ...
            swap_char(&result[i], &result[index]);
            result[i] ^= key[randint(0, key_len - 1)];
        }
        if (i - 1 >= 0)
        {
...
void solve(char *str, int str_len, char *key, int key_len, char *result)
{
    ...
    seed(key, key_len);
    int *index_list = (int *)calloc(str_len * 5, sizeof(int));

    int head = 0;
    for (int i = 0; i < str_len; i++)
    {
        index_list[head++] = randint(0, key_len - 1);
        if (i + 1 <= str_len - 1)
        {
            index_list[head++] = randint(i + 1, str_len - 1);
            index_list[head++] = randint(0, key_len - 1);
        }
        if (i - 1 >= 0)
        {
            index_list[head++] = randint(0, i - 1);
            index_list[head++] = randint(0, key_len - 1);
        }
    }
    ...
    for (int i = str_len - 1; i >= 0; i--)
    {
        ...
        {
            result[i] ^= result[i - 1];
            result[i] ^= key[index_list[--head]];
            int index = index_list[--head];
            swap_char(&result[i], &result[index]);
        }
        if (i + 1 <= str_len - 1)
        {
...
#define KEY_LENGTH (70)
    ...
    char flag[FLAG_LENGTH + 10] = {0};
    char key[] = "the_sha256_of_the_flag_is_not_Ne44roib3Ko6rpDnErL7UzoTbfIlHqsvFCHJczdU";
    char ans[] = "\x09\x1f\x4d\x03\x2f\x00\x53\x14\x3c\x07\x52\x3a\x1a\x70\x0c\x51\x64\x13\x63\x1b\x46\x73\x52\x6a\x79\x6f\x5c\x4f\x3f\x02\x43\x63\x32\x7b\x3c\x7e\x2f\x4b\x16\x31\x7e\x00\x3b\x33\x14\x16\x6a\x5c\x51\x3e";
